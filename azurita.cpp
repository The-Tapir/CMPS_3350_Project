...
//
// MAIN GOAL -------> display square box(car) to display on track in 3rd person
//
//INDIVIDUAL CODE starts at line 76
...
public:
    ...
    Car() {
        //constructor
        MakeVector(0.0f, 0.5f, 7.0f, position); // this is to set the postion of the car
        MakeVector(0.8f, 0.5f, 1.5f, size); // this is the dimension of the car. ----> hieght, width, length
        MakeVector(0.0f, 0.0f, 0.0f, rotation); // goal of this is to try to rotate the car itself ----> still needs to be worked on
    }
...
void drawCar() {
    glColor3f(0.8f, 0.2f, 0.2f); // this is the color of the car. RED. 
    glPushMatrix();
    glTranslatef(car.position[0], car.position[1], car.position[2]);
    box(car.size[0], car.size[1], car.size[2]);
    glPopMatrix();
}
...
//below is code added to display the car. 
