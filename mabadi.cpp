...
OBJMesh loadOBJ(const std::string& racetrack) {
    ...
    while (std::getline(file, line)) {
        ...
        if (token == "v") {
            Vertex vertex;
            iss >> vertex.position[0] >> vertex.position[1] >> 
                vertex.position[2];
...
//This was meant to be the code to test the mouse timer functions but it ended
//calculating the mouse distance
int time_since_mouse_moved(const bool get) {
    static std::chrono::steady_clock::time_point lastMouseMoveTime = 
    ...
    std::chrono::duration<double> time_span = 
        ...
        <std::chrono::duration<double>>(now - lastMouseMoveTime);
...
//These functions below here to make the guard rails I could not really 
//figure out I took them from the car.cpp framework. 
//I was not able to make them display.
struct Vec3 {
    float x, y, z;
};
...
void drawTrack() {
    glBegin(GL_TRIANGLES);
    for (std::vector<unsigned int>::size_type i = 0; i 
            < terrain.indices.size(); i += 3) {
...
void drawTree3D() {
    ...
    glColor3f(0.5f, 0.35f, 0.05f); // Just draw the color of the trunk a 
                                   // Brown color
    ...
    glVertex3f(0.0f, trunkHeight + leavesHeight, 0.0f); 
    //Above function will measure the very top of the tree or leaves.
...
void drawSphere
(float radius, int slices, int stacks, GLfloat r, GLfloat g, GLfloat b) {
    glColor3f(r, g, b);
    glBegin(GL_TRIANGLE_FAN);
...
void drawStoplight() {
    ...
    // This function below will draw the color of 
    // the pole and also make it into the shape we want it which is a cylinder.
...
//This function below just pretty much adjusts the positions of functions and
//we call startline, stoplight, moon, and stars in here. 
//Then we call drawScene in in car.cpp
void drawScene() {
    glPopMatrix();
    glPushMatrix();
